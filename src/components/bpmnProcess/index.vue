			...
			<el-form :inline="true" :model="processForm" :rules="rules" ref="processFormRef">
				<el-form-item class="process-info">
					<el-form-item label="流程Id" prop="id" v-show="false">
						<el-input type="hidden" size="small" v-model="processForm.id" placeholder="流程id"></el-input>
					</el-form-item>
					<el-form-item label="流程名称" prop="name">
						<el-input size="small" v-model.trim="processForm.name" @input="processForm.name=processForm.name.trim()" placeholder="请输入流程名称"></el-input>
					</el-form-item>
					<el-form-item label="流程编码" prop="key" v-show="modelData.type=='modify'">
						<el-input size="small" disabled v-model="processForm.key" placeholder="流程编码由系统自动生成" @input="processKeyChange"></el-input>
					</el-form-item>
					<el-form-item label="所属分类" prop="category">
						<el-select v-model="processForm.category" class="item-width" placeholder="请选择所属分类" size="small" no-data-text="暂无所属分类数据" popper-class="down-drop-tree" :popper-append-to-body="false">
							<el-option v-for="(item, index) in categoryTreeData" :key="index" :label="item.title" :value="item.id"></el-option>
						</el-select>
					</el-form-item>
					<el-form-item label="获取表单" prop="formId">
						...
						>
							<el-option v-for="(item, index) in formTreeData" :key="item.id" :label="item.title" :value="item.id"></el-option>
						</el-select>
					</el-form-item>
				</el-form-item>
				<el-form-item class="canvas-form-item" >
					<div class="table-box" ref="canvasContainer" :class="{canvasHeightAdjust:canvasHeight}">
						...
						<div class="table-cell canvas-content-box">
							<div class="canvas-container">
								<el-form-item class="editor-box" prop="editor">
									<el-input type="hidden" style="display: none;" v-model="processForm.editor"></el-input>
								...
								<div class="top-right">
									<ul class="right-top-box">
										...
										<li class="item-list" title="键盘快捷键" v-if="!canvasHeight" @click="keyboardEvent"><i class="keyboard-icon el-icon-custom-keyboard"></i></li>
										<li class="item-list" :title="canvasHeight?'退出全屏':'全屏'" v-if="enabledFullscreen" @click="fullScreenToggle()">
											<i :class="{'el-icon-custom-fullscreen':!canvasHeight,'el-icon-custom-outfullscreen':canvasHeight}"></i>
										</li>
									</ul>
									<el-dialog v-if="keyboardDialog" 
								...
								<div class="bottom-right">
									<ul class="bottom-right-box">
										<li class="item-list" @click="handlerZoom(0)"><i title="恢复" class="el-icon-custom-aim"></i></li>
										<li class="item-list" @click="handlerZoom(0.1)"><i title="放大" class="el-icon-custom-zoomin"></i></li>
										<li class="item-list" @click="handlerZoom(-0.1)"><i title="缩小" class="el-icon-custom-zoomout"></i></li>
									</ul>
								</div>
								<transition name="scale" >
									<div class="zoom-scale-view" v-show="scale!=1">
										<span>缩放：</span><span>{{scalePercent}}</span>
									</div>
								</transition>
							</div>
						</div>
...
//				this.vloading = true
				...
				this.$api.process.processNewSave(this.modelData.data).then(res => {
					if(type == 'depoly'){
			...
			processDeploy(){
				//发布
				this.$api.process.processDeploy(this.modelData.data.id).then((res)=>{
			...
			getModelData(row) {
				console.log(row);
				let result = (row.allElement || row.editor)?this.setRowProcess(row):this.$api.process.getModelById({
					id: row.id
				}).then(res => {
			...
			getCategory(row) {
				this.$api.process.getProcessType({
//					systemCode: this.processForm.processSystem,
			...
			getFormListData() {
				...
				this.$api.process.getFormSelect({
					isPublishVersion: 1,
					formId:formId
				}).then(res => {
			...
			async handleSubmit(type) {
				...
				this.processForm.editor = this.elementRegistry.getAll()[0].children.length ? (this.processForm.svg = svg) && this.replaceXml(xml) : ''
			...
			async createModifyDiagram(xml) {
				if(!xml) {
					...
					try {
						...
						this.processForm.editor = await (require('@/components/bpmnProcess/plugins/newDiagram').default)(processId,'','','')
						console.log(this.processForm.editor)
					} catch(err) {
						console.log(err)
					}
					...
					//回填
					this.processForm.editor = xml && xml.replace(/CDATA/g,"").replace(/\<\!\[\[/g,'').replace(/\]\]\>/g,"")
					console.log(this.processForm.editor)
				}
...
//					this.getCompleteNodeId()
			...
			setNodeColor(nodes, colorsJson){
				...
				nodes && nodes.length && nodes.forEach((elementOrId)=>{
					let elements = elementOrId.id?elementOrId : this.elementRegistry.get(elementOrId)
